    ...
    public extension ImageProcessor {
        func downsampleImage(to dimension: CGSize) throws -> CGImage? {
            ...
            let downsampleOptions = [
                /// kCGImageSourceCreateThumbnailFromImageAlways indicates that a thumbnail should be created from the
                /// image
                /// itself, not from any embedded thumbnail image that might be present.
                kCGImageSourceCreateThumbnailFromImageAlways: true,

                /// When this option is set to true, it instructs the Image I/O framework to decode and cache the image
                /// in
                /// full immediately upon creation,
                /// rather than delaying the decoding process until the image is actually rendered or accessed.
                /// Setting this option to true can lead to increased memory usage because the decoded image data is
                /// stored
                /// in memory right after the image is created.
                kCGImageSourceShouldCacheImmediately: false,

                /// This option, when set to true, ensures that the thumbnail is generated with the proper orientation
                /// based
